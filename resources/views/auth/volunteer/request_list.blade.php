...
@section('content')
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">List of Request</h1>
    </div>
                                ...
                                <table class="table table-striped table-hover table-bordered" id="ResourceRequest">
                                    <thead>
                                        <tr>
                                            ...
                                            <th class="text-center" style="width:25%"> Requester </th>
                                        ...
                                        @foreach($resource_request as $key => $data)
                                            ...
                                            <tr>
                                                ...
                                                <td class="text-center text-md"> 
                                                    @foreach($school as $key => $data2)
                                                        @if($data2->id == $data->schoolID)
                                                            @php
                                                                $requester = $data2->schoolName . " | " .  $data2->address . ", " . $data2->city 
                                                            @endphp
                                                            {{ $data2->schoolName }} <br>
                                                            ({{ $data2->address }}, {{ $data2->city }})
                                                        @endif
                                                    @endforeach
                                                </td>
                                                <td class="text-center">
                                                ...
                                                <td class="text-center"> 
                                                    <a href="#offerModal" class="offer text-lg" title="Detail" data-bs-toggle="modal"
                                                    data-reqid="{{ $data->requestID }}"
                                                    data-reqdate="{{ $data->requestDate }}"
                                                    data-desc="{{ $data->description }}"
                                ...
                                <table class="table table-striped table-hover table-bordered" id="ResourceRequest">
                                    <thead>
                                        <tr>
                                            ...
                                            <th class="text-center" style="width:15%"> Request Date </th>
                                            <th class="text-center" style="width:25%"> Requester </th>
                                            <th class="text-center"> Proposed Date & Time </th>
                                            <th class="text-center" style="width:10%"> Detail </th>
                                            <th class="text-center" style="width:10%"> Offer </th>
                                        </tr>
                                    </thead>
    ...
    <div id="offerModal" class="modal fade">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">New Help Offer</h5>
                    <button class="close" type="button" data-bs-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <form class="needs-validation" action="{{ route('create_offer') }}" method="POST">
                    @csrf
                    <div class="modal-body">
                        <div class="col-12 pt-3">
                        ...
                        <div class="col-12 pt-3">
                            <label for="requester"> Requester Data: </label>
                        ...
                        <div class="col-12 pt-3">
                            ...
                            <textarea type="text" class="form-control" id="remarks" name="remarks" rows="4" required></textarea>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button class="btn btn-secondary" type="button" data-bs-dismiss="modal">Cancel</button>
                        <button class="btn btn-success" type="submit"> Submit New Offer </button>
                    </div>
                </form>
            </div>
        </div>
...
@section('scripts')
    ...
    <script>
        ...
        $('#offerModal').on('show.bs.modal', function(e) {
            ...
            //get data-id attribute of the clicked element
            var reqID = $(e.relatedTarget).data('reqid');
            var reqDate = String($(e.relatedTarget).data('reqdate'));
            var desc = $(e.relatedTarget).data('desc');
            var requester =  $(e.relatedTarget).data('requester');
